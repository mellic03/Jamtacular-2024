...
WorldQueryResult.origin = new vec2(0, 0);
WorldQueryResult.hit = new vec2(0, 0);
WorldQueryResult.normal = new vec2(0, 0);
WorldQueryResult.dist = 0.0;
export default class WorldQuery {
    ...
    static raycast(data, origin, dir) {
        ...
        const RUSS = vec2.tmp();
        RUSS.setXY(Math.sqrt(1 + (dy / dx) * (dy / dx)), Math.sqrt(1 + (dx / dy) * (dx / dy)));
        let row = int(y);
        let col = int(x);
        // console.log(row, col);
        let dxdist = 0.0;
